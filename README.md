...
- [ ] Explain the difference between `Relational Databases` and `SQL`.
    Relational Databases are a collection of tables that hold data, while SQL is a Database management system. this means that DBMS's main purpose is to manage and interact with the database itself.

- [ ] Why do tables need a `primary key`?
      it is needed as a way to have a unique idenitfier every single record inside of a relational database.

- [ ] What is the name given to a table column that references the primary key on another table.
    foreign keys

- [ ] What do we need in order to have a _many to many_ relationship between two tables.
  a third table with foreign keys for the two tables.
